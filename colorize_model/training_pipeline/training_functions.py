...
def train_model(generator, discriminator, opt_g, opt_d, criterion, train_dataloader, val_batch,
    ...
    for epoch in trange(1, n_epochs + 1):
        ...
        save_model_onnx(generator, img_l, "image_l", "image_ab", **models_metadata["generator"])
        wandb.save(models_metadata["generator"]["path_to_save"], policy="now")

        save_model_onnx(discriminator, fake_image, "image", "patch", **models_metadata["discriminator"])
        wandb.save(models_metadata["discriminator"]["path_to_save"], policy="now")
